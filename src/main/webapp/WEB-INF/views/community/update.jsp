    ...
    <section class="meetings-page" id="meetings">
        <div class="container"
            ...
            <div id="smarteditor" style="padding: 10px;">
                ...
                <textarea name="editortx" id="editortx2" cols="20" rows="10"
                    style="width: 100%; height: 412px;;"></textarea>
            </div>
            <div id="se2_sample" style="margin:10px 0;">
                <input type="button" onclick="change();" value="수정 완료" class="btn btn-outline-dark">
            </div>
...
<script>
    const titleI = document.querySelector("#titleInput")
    ...
    window.addEventListener("DOMContentLoaded", function(){
        ...
        titleI.value = boardContent.title
        contexts = boardContent.content
        textAre.value = contexts
    ...
    function change() {
        const boardNum = JSON.parse(localStorage.getItem("watchNum"))
        oEditors.getById["editortx2"].exec("UPDATE_CONTENTS_FIELD", []);
        //스마트 에디터 값을 텍스트컨텐츠로 전달
        var content = document.getElementById("smartEditor").value;
        const context = document.getElementById("editortx2").value;
        ...
        changeToLocal(changeContent, boardNum-1)
        window.location.href = 'community-m.html'
    }

    function changeToLocal(contextss, index) {
        let notice = JSON.parse(localStorage.getItem('community')) || [];
        notice[index].title = contextss.title
        notice[index].content = contextss.content
        localStorage.setItem('community', JSON.stringify(notice));
    }
